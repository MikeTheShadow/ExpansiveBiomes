...
{
    //private static final BlockState field_181629_a = Blocks.OAK_LOG.getDefaultState();
    //private static final BlockState field_181630_b = Blocks.OAK_LEAVES.getDefaultState();
    private static final BlockState field_181629_a = ExpansiveBiomes.blockListLog.get(0).getDefaultState();
    private static final BlockState field_181630_b = ExpansiveBiomes.blockListLeaves.get(0).getDefaultState();



    private final boolean useExtraRandomHeight;
    public MapleTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> p_i49917_1_, boolean p_i49917_2_, boolean p_i49917_3_)
    {
            ...
            if (!flag) {
                        ...
                        for(int j1 = position.getZ() - k2; j1 <= position.getZ() + k2; ++j1) {
                            ...
                            if (Math.abs(i3) != k2 || Math.abs(k1) != k2 || rand.nextInt(2) != 0 && j2 != 0) {
                                ...
                                if (isAirOrLeaves(worldIn, blockpos)) {
                                    this.setLogState(changedBlocks, worldIn, blockpos, field_181630_b, p_208519_5_);
                                }
                            }
                        }
                    }
                ...
                for(int i2 = 0; i2 < i; ++i2) {
                    if (isAirOrLeaves(worldIn, position.up(i2))) {
                        this.setLogState(changedBlocks, worldIn, position.up(i2), field_181629_a, p_208519_5_);
                    }
                }
